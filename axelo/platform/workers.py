...
class ReverseWorker(WorkerBase):
    def __init__(
        ...
        *,
        queue_name: str = "default",
        region: str = "global",
        orchestrator_factory: Callable[[], MasterOrchestrator] = MasterOrchestrator,
    ) -> None:
...
class CrawlWorker(WorkerBase):
    ...
    async def _execute_and_ingest(
        ...
        target = TargetSite(url=spec.site_url, session_id=job_id[:8], interaction_goal="platform crawl", output_format=spec.output_format)
    ...
    async def run_once(self) -> bool:
        ...
        try:
            async with BrowserDriver(settings.browser, settings.headless) as driver:
                ...
                await browser_state_store.persist_context(job.job_id[:8], spec.refresh_url, driver.context, session_state)
            account.session_state_ref = str(store.state_path(job.job_id[:8]))
